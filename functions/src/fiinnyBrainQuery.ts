...
class TimeEngine {
    ...
    static filterByTimeOfDay(expenses: ExpenseItem[], period: string): ExpenseItem[] {
        return expenses.filter((e) => {
            ...
            if (period === "night") return hour >= 21 || hour < 6;
            return false;
        });
    }
}
...
class TrendEngine {
    static calculateGrowthRate(current: ExpenseItem[], previous: ExpenseItem[]): number {
    ...
    static analyzeTrendDirection(growth: number): string {
        if (growth > 10) return "increasing significantly";
        if (growth > 0) return "slightly increasing";
        if (growth < -10) return "decreasing significantly";
        if (growth < 0) return "slightly decreasing";
        return "stable";
    }

    static detectAnomaly(current: ExpenseItem[], history: ExpenseItem[]): { message: string } {
        const currentTotal = current.reduce((sum, e) => sum + e.amount, 0);
...
class InferenceEngine {
    static inferComplexIntent(expenses: ExpenseItem[], intent: string): ExpenseItem[] {
        if (intent === "hospital_travel") {
            return expenses.filter((e) =>
                e.labels?.some((l) => l.toLowerCase().includes("hospital") || l.toLowerCase().includes("medical"))
            );
        }
        return [];
    }
    ...
    static inferContext(expenses: ExpenseItem[], context: string): ExpenseItem[] {
        return expenses.filter((e) =>
            e.labels?.some((l) => l.toLowerCase().includes(context))
        );
    }

    static inferByCategory(expenses: ExpenseItem[], category: string): ExpenseItem[] {
        return expenses.filter((e) =>
            e.labels?.some((l) => l.toLowerCase().includes(category))
        );
    }
}
...
async function processQuery(
    ...
    if (queryLower.includes("increas") || queryLower.includes("decreas") || queryLower.includes("trend")) {
        ...
        const lastMonth = expenses.filter((e) =>
            isWithinInterval(e.date, { start: startOfMonth(subMonths(now, 1)), end: endOfMonth(subMonths(now, 1)) })
        );
