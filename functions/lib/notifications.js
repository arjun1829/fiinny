...
const COPY_ENGAGEMENT = {
    ...
    streak: [
        (n) => `🔥 ${n} Day Streak! Keep the fire burning!`,
        (n) => `You're on a roll! ${n} days in a row. 🚀`,
        (n) => `Don't break the chain! Day ${n} is here.`,
    ],
    morning: [
...
function getDailyMessage(streak) {
    ...
    if (streak > 2 && rand > 0.7) {
        // Streak oriented
        const tmpl = COPY_ENGAGEMENT.streak[Math.floor(Math.random() * COPY_ENGAGEMENT.streak.length)];
        return {
            title: "Streak Alert! 🔥",
...
export const onGroupExpenseCreated = onDocumentCreated({
    ...
    const recipients = friendIds.filter((id) => id !== payerId);
    ...
    const tokens = [];
    const fetchPromises = recipients.map(async (uid) => {
