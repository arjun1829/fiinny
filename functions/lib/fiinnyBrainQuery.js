...
export const fiinnyBrainQuery = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        // Fetch recent chat history for conversation memory (last 10 messages)
        const chatSnap = await db.collection("users").doc(userPhone).collection("brain_chat")
            .orderBy("timestamp", "desc")
            .limit(10)
            .get();

        const chatHistory = [];
        chatSnap.docs.reverse().forEach(doc => {
            const d = doc.data();
            if (d.isUser) {
...
- Conversation Memory: You can remember previous messages in this conversation. Use context from earlier messages when the user says things like "yes go ahead" or "do it".
...
- Use 'splitExpense' when the user wants to split a cost with friends.`
        ...
        if (message.tool_calls && message.tool_calls.length > 0) {
            ...
            // Get final response from OpenAI with full context
        ...
        res.json({ response: `Fiinny Brain Error: ${error.message}` });
    }
});

async function handleToolCall(userPhone, name, args) {
    if (name === "addExpense") {
...
async function findFriendIds(userPhone, names) {
