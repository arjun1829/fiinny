...
import {
  ...
  AlertCircle, PowerOff, RefreshCw,
} from "lucide-react";
import {
  fetchAllDiscounts,
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col bg-white shadow-2xl">
        {/* Header */}
        <div className="flex items-start justify-between gap-3 border-b border-outline-variant/30 px-6 py-5 shrink-0">
          <div className="min-w-0">
            ...
            <p className="text-xs text-on-surface-variant pl-10">
              {row.ownerPhone} · <span className="capitalize">{row.ownerType}</span>
            </p>
          </div>
          <button onClick={onClose} className="mt-0.5 shrink-0 rounded-xl p-2 hover:bg-surface-container text-on-surface-variant">
            <X className="h-5 w-5" />
          </button>
        </div>

        {/* Body — reuse the same DiscountPanel as sellers use */}
