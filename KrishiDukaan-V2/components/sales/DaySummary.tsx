...
import { Clock, MapPin, Store, CheckCircle, AlertCircle } from 'lucide-react';
...
export default function DaySummary({ session, visitCount }: DaySummaryProps) {
  ...
  const hasFailed  = !!session.calculationError;
  ...
  return (
    <div className={`rounded-2xl p-4 ring-1 ${isActive ? 'bg-primary/5 ring-primary/25' : 'bg-white ring-outline/10'}`}>
      ...
      {/* Distance */}
      <div className={`mt-3 flex items-start gap-3 rounded-xl px-4 py-3 ${hasFailed ? 'bg-red-50' : 'bg-surface-container-low'}`}>
        <MapPin className={`mt-0.5 h-4 w-4 shrink-0 ${hasFailed ? 'text-red-400' : 'text-primary'}`} />
        <div className="min-w-0 flex-1">
          ...
          {isActive ? (
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-red-500 leading-snug">{session.calculationError}</p>
            </div>
          ) : (
