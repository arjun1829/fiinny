...
class _SettleUpSheetV2State extends State<SettleUpSheetV2> {
  ...
  Set<String> get _eligibleSelectableIds => widget.outstandingByGroup.entries
      .where((entry) =>
          entry.value != 0 && (entry.value > 0) == _controller.isReceiveFlow)
      .map((entry) => entry.key)
      .toSet();
  ...
  void _syncFromController() {
    ...
    if ((parsed - controllerAmount).abs() > 0.01) {
      _amountCtrl.text =
          controllerAmount > 0 ? _formatAmount(controllerAmount) : '';
    ...
    final match = options
        .indexWhere((opt) => (opt.amount - controllerAmount).abs() < 0.01);
    setState(() {
  ...
  List<AmountQuickChipOption> _buildQuickOptions() {
    ...
    void addOption(String label, double rawValue) {
      ...
      if (options.any((opt) => (opt.amount - rounded).abs() < 0.01)) {
        return;
      }
    ...
    try {
      if (_controller.isReceiveFlow) {
        ...
        await widget.onMarkReceived(
            SettleUpResult(amount: amount, allocations: allocations));
        if (!mounted) {
          return;
        }
        await showDialog(
  ...
  Map<String, double> _allocateAmount(double amount) {
    ...
    if (remaining > 0.01 && allocations.isNotEmpty) {
      ...
      allocations[lastKey] =
          double.parse((allocations[lastKey]! + remaining).toStringAsFixed(2));
    }
    return allocations;
  }
  ...
  Widget _buildHeader() {
    ...
    final getBack = outstanding
        .where((value) => value > 0)
        .fold<double>(0.0, (sum, value) => sum + value);
    final youOwe = outstanding
        .where((value) => value < 0)
        .fold<double>(0.0, (sum, value) => sum + value.abs());
    final eligible = _eligibleSelectableIds;
    final allSelected = eligible.isNotEmpty &&
        eligible.every(_controller.selectedGroupIds.contains);
    final someSelected =
        _controller.selectedGroupIds.isNotEmpty && !allSelected;
    ...
    return Column(
      ...
      children: [
        ...
        ...widget.groupDisplays.map((group) {
          ...
          final enabled = (group.amount >= 0) == _controller.isReceiveFlow &&
              group.amount.abs() > 0.0;
          return SettleGroupRow(
            ...
            leading:
                group.avatarUrl != null && group.avatarUrl!.startsWith('http')
                    ? CircleAvatar(
                        radius: 22,
  ...
  Widget _buildAmountSection() {
    ...
    final summaryColor = _controller.isReceiveFlow
        ...
        AmountQuickChips(
          ...
          onSelected: (value) {
            ...
            setState(() {
              ...
              _selectedChipIndex = quickOpts
                  .indexWhere((opt) => (opt.amount - value).abs() < 0.01);
            });
          },
          onClear: () {
...
class _FriendHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final neutralColor = textTheme.bodySmall?.color?.withValues(alpha: .72) ??
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      buildChip('You get back', getBackAmount,
                          Theme.of(context).colorScheme.primary),
                      buildChip(
                          'You owe', oweAmount, neutralColor ?? Colors.black),
                    ],
                  ),
                ],
              ),
...
class _SummaryBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final valueColor = isReceiveFlow
        ...
        : theme.textTheme.bodyLarge?.color?.withValues(alpha: .72) ??
