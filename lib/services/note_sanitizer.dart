...
class NoteSanitizer {
  ...
  static String _truncate(String s, int max) => s.length <= max ? s : (s.substring(0, max - 1) + '…');

  static String _fmtAmt(double v) {
    // Indian clustering without intl dep inside sanitizer
    final parts = v.toStringAsFixed(v.truncateToDouble() == v ? 0 : 2).split('.');
    final n = parts[0];
    if (n.length <= 3) {
    ...
    final headGrouped = head.replaceAll(RegExp(r'(\d)(?=(\d{2})+(?!\d))'), r'$1,');
    return parts.length == 2 ? '$headGrouped,$tail.${parts[1]}' : '$headGrouped,$tail';
  }

  static String _shortRef(String r) => (r.length <= 12) ? r : '${r.substring(0, 4)}…${r.substring(r.length - 4)}';
  ...
  static int _countRemoved(String raw, String cleaned) {
    ...
    final cleanLines = cleaned.split('•').where((l)=>l.trim().isNotEmpty).length;
