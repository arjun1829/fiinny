...
class CommonRegex {
  static final _amount = RegExp(
      r'(?:INR|Rs\.?|₹|\$|USD|EUR|GBP|AED)\s?([\d,]+(?:\.\d{1,2})?)',
      caseSensitive: false);
  static final _last4 = RegExp(
      r'(?:ending|xx|XXXX|last\s?digits?|last\s?4)\s?(\d{4})',
      caseSensitive: false);
  ...
  static TxChannel detectChannel(String text) {
    ...
    if (t.contains('upi')) {
      return TxChannel.upi;
    }
    if (t.contains('credit card') ||
        t.contains('debit card') ||
        t.contains('card ')) {
      return TxChannel.card;
    }
    if (t.contains('neft') || t.contains('imps') || t.contains('rtgs')) {
      return TxChannel.bank;
    }
    if (t.contains('wallet') ||
        t.contains('paytm') ||
        t.contains('amazon pay')) {
      return TxChannel.wallet;
    }
    if (t.contains('atm')) {
      return TxChannel.atm;
    }
  ...
  static ({String category, String subcategory}) categoryHint(String text,
      {String? merchantName, bool isP2P = false, bool isCard = false}) {
    final t = text.toLowerCase();
    ...
    if (t.contains('irctc') ||
        ...
        t.contains('ola') ||
        t.contains('air')) {
      return (category: 'Travel', subcategory: 'general');
    }
    if (t.contains('electricity') || t.contains('bill') || t.contains('dth')) {
      return (category: 'Bills', subcategory: 'utilities');
    }
    if (t.contains('salary')) {
      return (category: 'Salary', subcategory: 'salary');
    }
    if (isCredit(text)) {
      return (category: 'Income', subcategory: 'general');
    }
