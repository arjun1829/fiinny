...
// Delegate to shared filter helper (with safe fallback)
  ...
  Future<void> _handleOne({
    ...
    final existingDocId = await CrossSourceReconcile.maybeMerge(
      userId: userPhone,
      direction: direction,
      amount: amount,
      timestamp: ts,
    ...
    if (existingDocId != null) {
      _log('merged into existing $direction doc=$existingDocId via reconcile');
      return;
    }
    ...
    if (direction == 'debit') {
      ...
      // Categorize + recurring/loan hooks (best-effort)
      try {
