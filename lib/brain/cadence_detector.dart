...
class CadenceDetector {
  ...
  static final _autopayKw = RegExp(r'\b(standing instruction|si|ecs|mandate|autopay|auto[- ]?debit|upi[- ]?auto)\b', caseSensitive: false);
  ...
  static List<RecurringItem> detect(List<ExpenseItem> expenses) {
    ...
    groups.forEach((key, list) {
      ...
      for (final e in list) {
        ...
        isSubscription |= tags.contains('subscription') || _subMerchants.hasMatch(n);
      }
      String type;
      if (isLoan) type = 'loan_emi';
      else if (isSubscription) type = 'subscription';
      else if (isAutopay) type = 'autopay';
      else type = 'subscription'; // default bucket

      // approx amount (use avg of last 3 or last)
      final tail = list.sublist(max(0, list.length - 3));
      final monthly = tail.fold<double>(0.0, (a, b) => a + b.amount) / tail.length;
  ...
  static String _merchantOf(ExpenseItem e) {
    ...
    final m = (meta != null ? (meta['merchant'] as String?) : null) ?? e.label ?? e.category ?? '';
