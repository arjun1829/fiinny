...
class GmailConnector extends SourceConnector {
  ...
  Future<List<RawTransactionEvent>> backfill({int days = 90}) async {
    if (_api == null || !region.allowGmailIngestion) {
      return [];
    }
    ...
    // Fetch list
    final response =
        await _api!.users.messages.list('me', q: query, maxResults: 100);
  ...
  RawTransactionEvent? _parseEmail(gmail.Message msg) {
    ...
    if (!_isFinancialEmail(subject, from, bank)) {
      return null;
    }
  ...
  double? _extractAmount(String body) {
    ...
    if (region.countryCode == 'IN') {
      ...
      pattern = r'(?:' +
          RegExp.escape(region.currencySymbol) +
          r')\s*([0-9,]+(?:\.[0-9]{1,2})?)';
    }
  ...
  String? _extractCurrency(String body) {
    if (body.contains('USD') || body.contains('\$')) {
      return 'USD';
    }
    if (body.contains('EUR') || body.contains('€')) {
      return 'EUR';
    }
    if (body.contains('INR') || body.contains('₹')) {
      return 'INR';
    }
    return null;
  }
  ...
  String? _extractAccountHint(String body) {
    final match =
        RegExp(r'(?:ending|x|xx)\s*[ -]?x*([0-9]{4})\b', caseSensitive: false)
            .firstMatch(body);
  ...
  String? _extractMerchant(String body, String subject) {
    ...
    final match =
        RegExp(r'\b(?:at|to)\s+([A-Za-z0-9 ]{2,20})', caseSensitive: false)
            .firstMatch(body);
    if (match != null) {
