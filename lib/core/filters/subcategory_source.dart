...
class SubcategorySource {
  ...
  Future<List<String>> getSubcategories(String category) async {
    ...
    if (c == 'food') return ['Dining', 'Groceries', 'Snacks'];
    if (c == 'travel') return ['Cabs', 'Flights', 'Trains', 'Fuel'];
    if (c == 'bills') return ['Electricity', 'Internet', 'Mobile', 'DTH'];
    if (c == 'shopping') return ['Online', 'Offline', 'Electronics', 'Fashion'];
    return const <String>[];
  }
}
