...
class AddTransactionScreen extends StatefulWidget {
  ...
  const AddTransactionScreen({required this.userId, super.key});
...
class _AddTransactionScreenState extends State<AddTransactionScreen> {
  ...
  final Map<String, double> _splits = {}; // phone -> amount
  ...
  List<String> get _participants {
    if (_groupId != null && _groupId!.isNotEmpty) {
      final g = _groups.firstWhere((x) => x.id == _groupId,
          orElse: () => _GroupLite.empty());
  ...
  void _next() {
    ...
    if (_step == 0 && !_validStep0()) {
      return;
    }
    if (_step == 1 && !_validStep1()) {
      return;
    }
    if (_step == 2 && !_validStep2()) {
      return;
    }
    if (_step < 3) {
      setState(() => _step += 1);
  ...
  Future<String?> _uploadBillImage() async {
    ...
    try {
      final ref = FirebaseStorage.instance.ref(
          'users/${widget.userId}/tx_images/${DateTime.now().millisecondsSinceEpoch}.jpg');
  ...
  Future<void> _addFriendInline() async {
    ...
    final blacky = base.copyWith(
      colorScheme: base.colorScheme.copyWith(
        primary: base.colorScheme
            .onSurface, // controls focused indicators, selection, etc.
        secondary: base.colorScheme.onSurface,
        surface: base.colorScheme.surface,
      ),
      textButtonTheme: TextButtonThemeData(
        style:
            TextButton.styleFrom(foregroundColor: base.colorScheme.onSurface),
      ),
      elevatedButtonTheme: ElevatedButtonThemeData(
        style: ElevatedButton.styleFrom(
          backgroundColor: base.colorScheme.onSurface,
          foregroundColor: base.colorScheme.onInverseSurface,
          shape:
              RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        ),
      ),
      checkboxTheme: CheckboxThemeData(
        fillColor: WidgetStatePropertyAll(base.colorScheme.onSurface),
        checkColor: WidgetStatePropertyAll(base.colorScheme.onInverseSurface),
      ),
      ...
      switchTheme: SwitchThemeData(
        thumbColor:
            WidgetStateProperty.resolveWith((s) => base.colorScheme.onSurface),
        trackColor: WidgetStateProperty.resolveWith(
            (s) => base.colorScheme.onSurface.withValues(alpha: 0.25)),
      ),
    );
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (_type == 'credit') {
        ...
        if ((_groupId ?? '').isNotEmpty && friendIds.isEmpty) {
          groupId = _groupId;
        }
        ...
        final expense = ExpenseItem(
          ...
          date: DateTime.now(),
          payerId:
              _customSplit ? (_payerPhone ?? widget.userId) : widget.userId,
          friendIds: friendIds,
          settledFriendIds: const [],
          cardType: (_type == 'cc_spend' || _type == 'cc_bill')
              ? 'Credit Card'
              : null,
          cardLast4: (_type == 'cc_spend' || _type == 'cc_bill')
              ? _cardLast4Ctrl.text.trim()
              : null,
          isBill: isBill,
          groupId: groupId,
          imageUrl: billUrl,
          ...
          customSplits: (_customSplit && _participants.isNotEmpty)
              ? _normalizedSplits()
              : null,
        );

        // Prefer sync path if splitting (friend/group/custom)
        final needsSync = (_customSplit && _participants.isNotEmpty) ||
                  ...
                  _StepDetails(
                    ...
                    onFriend: (v) {
                      setState(() {
                        ...
                        _payerPhone = widget.userId;
                        _rebuildSplitControllers(
                            equalizeIfPossible: _customSplit);
                      });
                    },
                    onGroup: (v) {
                      setState(() {
                    ...
                    onPayerChanged: (p) {
                      setState(() {
                        _payerPhone = p;
                        if (_customSplit) {
                          _equalSplit(); // keep residual on payer
                        }
                      });
                    },
                      ...
                      children: [
                        ...
                        _Box(
                          child: TextField(
                            ...
                            decoration:
                                _inputDec(context).copyWith(counterText: ''),
                            inputFormatters: [
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          if (type != 'credit' &&
              kExpenseSubcategories[value] != null &&
              kExpenseSubcategories[value]!.isNotEmpty) ...[
            ...
            Container(
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  ...
                  items: kExpenseSubcategories[value]!
...
class _StepDetails extends StatelessWidget {
  ...
  String _nameForGroup(String? id) {
    if (id == null) return '';
    final g =
        groups.firstWhere((x) => x.id == id, orElse: () => _GroupLite.empty());
    return g.name;
  }
            ...
            child: Column(
              ...
              children: [
                ListTile(
                  ...
                  onTap: () =>
                      Navigator.of(sheetContext).pop(_SplitAddOption.friend),
                ),
                ListTile(
                  leading: const Icon(Icons.group_add_rounded),
                  title: const Text('Create Group'),
                  onTap: () =>
                      Navigator.of(sheetContext).pop(_SplitAddOption.group),
                ),
              ],
            ),
          );
              ...
              Expanded(
                child: _Box(
                  ...
                  child: TextField(
                    ...
                    decoration:
                        _inputDec(context).copyWith(hintText: 'Eg: Goa Trip'),
                    onChanged: (v) {
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                decoration:
                    _inputDec(context).copyWith(labelText: 'Group name'),
              ),
              const SizedBox(height: 12),
              Align(
                    ...
                    onPressed: _saving
                        ...
                        : () async {
                            ...
                            if (_selected.isEmpty) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                      content: Text('Pick at least 1 member')));
                              return;
                            }
                            Navigator.pop(
                                context,
...
class _GroupLite {
  ...
  final List<String> memberPhones;
  const _GroupLite(
      {required this.id, required this.name, required this.memberPhones});
  static _GroupLite empty() =>
      const _GroupLite(id: '', name: '', memberPhones: []);
}
