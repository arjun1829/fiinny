...
class AssetsScreen extends StatefulWidget {
  final String userId;
  const AssetsScreen({required this.userId, super.key});
  ...
  Future<void> _confirmDelete(AssetModel asset) async {
    ...
    if (ok == true) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text('Asset "${asset.title}" deleted!'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          final added = await Navigator.pushNamed(context, '/addAsset',
      ...
      body: FutureBuilder<List<AssetModel>>(
        ...
        builder: (context, snap) {
          final bottomInset =
              context.adsBottomPadding(extra: 80); // Extra for FAB
                  ...
                  SliverToBoxAdapter(
                    child: Container(
                      ...
                      child: ListView.separated(
                          ...
                          itemCount:
                              breakdown.length > 5 ? 6 : breakdown.length,
                            ...
                            return Container(
                                ...
                                child: Row(children: [
                                  ...
                                  Container(
                                      ...
                                      height: 8,
                                      decoration: BoxDecoration(
                                          color: _colorByKey(entry.key),
                                          shape: BoxShape.circle)),
                                  const SizedBox(width: 6),
                                  Text(_labelByKey(entry.key),
                                      style: const TextStyle(
                                          fontSize: 12,
                                          fontWeight: FontWeight.w600)),
                  ...
                  child: SizedBox(
                    ...
                    child: ListView(
                      ...
                      children: [
                        _FilterChip(
                            label: 'All',
                            selected: _segment == 'all',
                            onTap: () => setState(() => _segment = 'all')),
                        const SizedBox(width: 8),
                        _FilterChip(
                            label: 'Stocks',
                            selected: _segment == 'equity',
                            onTap: () => setState(() => _segment = 'equity')),
                        const SizedBox(width: 8),
                        _FilterChip(
                            label: 'MF/ETF',
                            selected: _segment == 'mf_etf',
                            onTap: () => setState(() => _segment = 'mf_etf')),
                        const SizedBox(width: 8),
                        _FilterChip(
                            label: 'Gold',
                            selected: _segment == 'gold',
                            onTap: () => setState(() => _segment = 'gold')),
                        const SizedBox(width: 8),
                        _FilterChip(
                            label: 'FD',
                            selected: _segment == 'fixed_deposit',
                            onTap: () =>
                                setState(() => _segment = 'fixed_deposit')),
                        const SizedBox(width: 8),
                        _FilterChip(
                            label: 'Real Estate',
                            selected: _segment == 'real_estate',
                            onTap: () =>
                                setState(() => _segment = 'real_estate')),
                        const SizedBox(width: 8),
                        _FilterChip(
                            label: 'Crypto',
                            selected: _segment == 'crypto',
                            onTap: () => setState(() => _segment = 'crypto')),
                        const SizedBox(width: 8),
                        _FilterChip(
                            label: 'Bank',
                            selected: _segment == 'cash_bank',
                            onTap: () =>
                                setState(() => _segment = 'cash_bank')),
                      ],
                    ),
                  ),
                ),
