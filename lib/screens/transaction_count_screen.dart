...
class _TransactionCountScreenState extends State<TransactionCountScreen>
  ...
  List<int> _buildBarData(
    ...
    final DateTime now = DateTime.now();
    if (period == "D") {
      final List<int> count = List.filled(24, 0);
      ...
      final List<int> count = List.filled(7, 0);
      for (var e in expenses) {
        count[e.date.weekday - 1] += 1;
      }
      for (var i in incomes) {
        count[i.date.weekday - 1] += 1;
      }
      return count;
    } else if (period == "M") {
      final int days = DateTime(now.year, now.month + 1, 0).day;
      final List<int> count = List.filled(days, 0);
      for (var e in expenses) {
        count[e.date.day - 1] += 1;
      }
      for (var i in incomes) {
        count[i.date.day - 1] += 1;
      }
      return count;
    } else if (period == "Y") {
      final List<int> count = List.filled(12, 0);
      for (var e in expenses) {
        count[e.date.month - 1] += 1;
      }
      for (var i in incomes) {
        count[i.date.month - 1] += 1;
      }
      return count;
    }
    return [];
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < barCount; i++) {
      if (_barCount[i] > maxVal) {
        maxVal = _barCount[i];
      }
    }
    final int ySteps = 5;
    final List<int> yAxisVals = List.generate(
      ySteps,
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          _miniAmount(
                              "Income", _incomeCount, Colors.green[700]!),
                          const SizedBox(width: 15),
                          _miniAmount(
                              "Expense", _expenseCount, Colors.red[600]!),
                        ],
                      ),
                    ),
                    const Text(
