...
class TransactionAmountScreen extends StatefulWidget {
  final String userId;
  const TransactionAmountScreen({super.key, required this.userId});
...
class _TransactionAmountScreenState extends State<TransactionAmountScreen>
  ...
  void _updateForPeriod({bool animate = true}) {
    final List<ExpenseItem> filteredExpenses =
        _filteredExpensesForPeriod(_period);
    final List<IncomeItem> filteredIncomes = _filteredIncomesForPeriod(_period);
    ...
    final barPair =
        _barDataCreditDebit(_period, filteredExpenses, filteredIncomes);
    _barDebit = barPair[0];
  ...
  List<IncomeItem> _filteredIncomesForPeriod(String period) {
    ...
    if (period == "D") {
      ...
      return _allIncomes.where((e) => e.date.year == now.year).toList();
    }
    return _allIncomes;
  }
  ...
  List<List<double>> _barDataCreditDebit(
    ...
    final DateTime now = DateTime.now();
    if (period == "D") {
      final List<double> debit = List.filled(24, 0.0);
      final List<double> credit = List.filled(24, 0.0);
      for (var e in expenses) {
        debit[e.date.hour] += e.amount;
      }
      for (var i in incomes) {
        credit[i.date.hour] += i.amount;
      }
      return [debit, credit];
    } else if (period == "W") {
      final List<double> debit = List.filled(7, 0.0);
      final List<double> credit = List.filled(7, 0.0);
      for (var e in expenses) {
        debit[e.date.weekday - 1] += e.amount;
      }
      for (var i in incomes) {
        credit[i.date.weekday - 1] += i.amount;
      }
      return [debit, credit];
    } else if (period == "M") {
      final int days = DateTime(now.year, now.month + 1, 0).day;
      final List<double> debit = List.filled(days, 0.0);
      final List<double> credit = List.filled(days, 0.0);
      for (var e in expenses) {
        debit[e.date.day - 1] += e.amount;
      }
      for (var i in incomes) {
        credit[i.date.day - 1] += i.amount;
      }
      return [debit, credit];
    } else if (period == "Y") {
      final List<double> debit = List.filled(12, 0.0);
      final List<double> credit = List.filled(12, 0.0);
      for (var e in expenses) {
        debit[e.date.month - 1] += e.amount;
      }
      for (var i in incomes) {
        credit[i.date.month - 1] += i.amount;
      }
      return [debit, credit];
    }
    return [[], []];
  ...
  String _xLabel(int idx, int barCount) {
    ...
    if (barCount >= 28 && barCount <= 31) {
      ...
      final List<int> labelIndices = [0, barCount - 1];
      final double step = (barCount - 1) / (numLabels - 1);
      for (int i = 1; i < numLabels - 1; i++) {
        final int nextIdx = (i * step).round();
  ...
  Widget build(BuildContext context) {
    ...
    final int ySteps = 5;
    final List<double> yAxisVals = List.generate(
      ySteps,
      (i) => (maxVal / (ySteps - 1)) * i,
    );

    final Widget filterBar = Container(
          ...
          return Expanded(
            child: GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color:
                      selected ? const Color(0xFF2C5AFF) : Colors.transparent,
          ...
          : SafeArea(
              ...
              child: SingleChildScrollView(
                padding: EdgeInsets.fromLTRB(
                    20, 20, 20, context.adsBottomPadding(extra: 20)),
                child: Column(
                                                ...
                                                children: [
                                                  ...
                                                  Row(
                                                    ...
                                                    children: List.generate(
                                                        barCount, (idx) {
                                                      final double valDebit =
                                                          _barDebit[idx];
                                                      final double valCredit =
                                                          _barCredit[idx];
                                                      final double normDebit =
                                                          maxVal == 0
                                                              ? 0
                                                              : valDebit /
                                                                  maxVal;
                                                      final double normCredit =
                                                          maxVal == 0
                                                              ? 0
                                                              : valCredit /
                                                                  maxVal;
                                                                    ...
                                                                    child:
                                                                      ...
                                                                      decoration:
                                                                        ...
                                                                        borderRadius:
                                                                          ...
                                                                          top: Radius.circular(
                                                                              7),
                                                                          bottom: Radius.circular(valDebit > 0
                                                                              ? 0
                                                                              : 7),
                                                                        ),
                                                                        color: Color(0xFF13CE66).withValues(
                                                                            alpha:
                                                                                0.85),
                                                                      ),
                                                                    ),
    ...
    return Row(
      children: [
        Icon(label == "Credit" ? Icons.arrow_downward : Icons.arrow_upward,
            size: 15, color: color.withValues(alpha: 0.83)),
...
class _HorizontalDottedLinesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < ySteps; i++) {
      final double y = size.height * i / (ySteps - 1);
      // Draw dotted line
      double startX = 0;
