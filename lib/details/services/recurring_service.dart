...
// Mark paid for a given scope (advances nextDueAt and optionally logs a payment).
  Future<void> markPaidScope(
    ...
    bool logPayment = true,
      ...
      _markPaidScope(scope, id,
          amount: amount, paidAt: paidAt, logPayment: logPayment);
...
// ==================== INTERNAL (scope helpers) ====================
  ...
  Future<String> addToGroup(
    ...
    if (participants.isNotEmpty) {
      final participantsTop =
          (data['participants'] as Map?)?.cast<String, dynamic>() ?? {};
  ...
  Stream<List<SharedItem>> streamByFriend(String userPhone, String friendId) {
    return _colFriend(userPhone, friendId)
        ...
        .snapshots()
        .map((snap) =>
            snap.docs.map(_mapDocSafe).whereType<SharedItem>().toList())
        .handleError((e, st) {
      if (kDebugMode) {
  ...
  Future<void> update(
    ...
    final data = _normalizeRuleForMerge(item.toJson())
      ..['updatedAt'] = FieldValue.serverTimestamp();

    if (!mirrorToFriend) {
      await _docFriend(userPhone, friendId, item.id)
  ...
  Future<void> patch(
    ...
    if (!mirrorToFriend) {
      await _docFriend(userPhone, friendId, itemId)
          .set(data, SetOptions(merge: true));
      return;
    }
  ...
  Future<void> updateRulePartial({
    ...
    required Map<String, dynamic>
        rulePatch, // e.g. {'frequency':'weekly','weekday':2}
    ...
    if (!recomputeNextDue) {
      await patch(userPhone, friendId, itemId, {'rule': rp},
  ...
  Future<void> softDelete(
    ...
    if (!mirrorToFriend) {
      await _docFriend(userPhone, friendId, id)
          .set(payload, SetOptions(merge: true));
      return;
    }
  ...
  Future<void> pause(String u, String f, String id) =>
      setStatus(u, f, id, 'paused');
  Future<void> resume(String u, String f, String id) =>
      setStatus(u, f, id, 'active');
  Future<void> end(String u, String f, String id) =>
      setStatus(u, f, id, 'ended');
  ...
  Future<void> updateRulePartialInGroup({
    ...
    if (!recomputeNextDue) {
      await patchInGroup(
          groupId: groupId, itemId: itemId, payload: {'rule': rp});
      return;
    }
  ...
  Future<void> patchMeta({
    ...
    await _docFriend(userId, friendId, itemId)
        .set({'meta': meta}, SetOptions(merge: true));
  }
  ...
  DateTime computeNextDue(RecurringRule rule, {DateTime? from}) {
    ...
    switch (rule.frequency) {
      ...
      case 'weekly':
        ...
        final int target =
            (rule.weekday ?? rule.anchorDate.weekday).clamp(1, 7);
      ...
      default:
        ...
        final DateTime candidate = DateTime(base.year, base.month, day);
