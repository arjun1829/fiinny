...
class _SalaryPredictorCardState extends State<SalaryPredictorCard> {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(
              child: Text('Salary & Payday Predictor',
                  style: Theme.of(context).textTheme.titleMedium)),
          if (_running)
            _chip(context, 'Analyzing…')
          else if (_done)
            _chip(context, 'Last run ✓')
          else
            _chip(context, 'Ready'),
          IconButton(
            icon: Icon(_expanded
        ...
        if (_expanded) ...[
          ...
          Wrap(spacing: 8, runSpacing: 8, children: [
            _pill(context, Icons.business_center_rounded, 'Employer',
                _employerName ?? '—'),
            _pill(context, Icons.attach_money_rounded, 'Avg (last 3)',
                _avgSalary == 0 ? '—' : '₹${_fmt(_avgSalary)}'),
            _pill(
                context,
                ...
                _amountStdPct == 0
                    ? '—'
                    : '${_amountStdPct.toStringAsFixed(1)}%'),
            _pill(context, Icons.emoji_events_rounded, 'Streak',
                _streakMonths > 0 ? '${_streakMonths}m' : '—'),
            _pill(context, Icons.verified_rounded, 'Confidence',
          ...
          Wrap(spacing: 8, runSpacing: 8, children: [
            ...
            _pill(
                ...
                _windowEnd == null
                    ? '—'
                    : '${_ddmmyy(_windowStart!)} → ${_ddmmyy(_windowEnd!)}'),
          ]),
          const SizedBox(height: 8),

          // Other employers (if any)
          if (_otherEmployers.isNotEmpty) ...[
            Text('Other employers detected',
                style: Theme.of(context).textTheme.labelLarge),
            const SizedBox(height: 6),
            Wrap(
              spacing: 6,
              runSpacing: 6,
              children: _otherEmployers
                  .map((e) => Chip(label: Text('${e.key} · ${e.value} hits')))
                  .toList(),
            ),
            const SizedBox(height: 8),
          ],
  ...
  Future<void> _runPrediction() async {
    ...
    try {
      ...
      while (true) {
        ...
        if (mounted) {
          setState(() => _status = 'Analyzing… $_scanned incomes');
        }
      }
      ...
      // ---------- group by employer ----------
      final groups =
          <String, List<({IncomeItem item, Map<String, dynamic> raw})>>{};
      ...
      for (final entry in groups.entries) {
        ...
        if (last.isAfter(primaryLast) ||
            (last.isAtSameMomentAs(primaryLast) &&
                list.length > (groups[primaryKey]?.length ?? 0))) {
          primaryKey = entry.key;
          primaryLast = last;
        }
      }

      final primary = (groups[primaryKey]!
        ..sort((a, b) => a.item.date.compareTo(b.item.date)));
      ...
      for (int idx = 0; idx < primary.length; idx++) {
        ...
        if (idx > 0) {
          diffs.add(primary[idx]
              .item
              .date
              .difference(primary[idx - 1].item.date)
              .inDays
              .abs());
        }
      }
      ...
      _streakMonths = _computeStreak(primary.map((r) => r.item.date).toList(),
          earlyDays: _earlyDays);
  ...
  String _employerFrom(Map<String, dynamic> raw, String note) {
    ...
    final by = RegExp(r'\b(by|from|frm)\s+([A-Za-z0-9&.\- ]{3,})',
            caseSensitive: false)
        .firstMatch(n);
    if (by != null) {
    ...
    final cmp =
        RegExp(r'\b([A-Z][A-Z0-9.&\- ]{3,})\b').firstMatch(n.toUpperCase());
      ...
      builder: (ctx) => SafeArea(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text('Early credit window (India)',
                style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            const Text(
                'Many salaries arrive a few days before 1st due to weekends/bank processing.'),
            const SizedBox(height: 12),
            Wrap(
              spacing: 8,
              runSpacing: 8,
