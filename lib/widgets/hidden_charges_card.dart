...
class HiddenChargesCard extends StatelessWidget {
      ...
      builder: (ctx, snap) {
        if (!snap.hasData) {
          return _card(context,
              title: 'Hidden Charges', body: const Text('Analyzing…'));
        }
        ...
        final List<ExpenseItem> items =
            docs.map((d) => ExpenseItem.fromFirestore(d)).toList();
        ...
        for (final e in items) {
          ...
          String t = 'Fee';
          final meta =
              (e.toJson()['brainMeta'] as Map?)?.cast<String, dynamic>();
          if (meta != null &&
              meta['feeType'] is String &&
              (meta['feeType'] as String).trim().isNotEmpty) {
            t = _title(meta['feeType'] as String);
          } else {
            // lightweight guess
            if (e.note.toLowerCase().contains('late')) {
              t = 'Late Fee';
            } else if (e.note.toLowerCase().contains('convenience')) {
              t = 'Convenience';
            } else if (e.note.toLowerCase().contains('processing')) {
              t = 'Processing';
            } else if (e.note.toLowerCase().contains('gst')) {
              t = 'GST';
            } else if (e.note.toLowerCase().contains('markup') ||
                e.note.toLowerCase().contains('forex')) {
              t = 'Forex Markup';
            }
          }
          ...
          body: Column(
            ...
            children: [
              ...
              if (total == 0)
                ...
                const Text(
                    'Tip: You can reduce convenience/forex charges by using UPI/zero-markup cards.'),
            ],
          ),
        );
      },
  ...
  Widget _card(BuildContext ctx,
      {required String title, String? subtitle, required Widget body}) {
    return Container(
