...
class InferenceEngine {
  ...
  static List<ExpenseItem> inferByCategory(List<ExpenseItem> expenses, String targetCategory) {
    ...
    return expenses.where((e) {
      ...
      final text = "${e.title ?? ''} ${e.note} ${e.labels.join(' ')}".toLowerCase();
      return keywords.any((k) => text.contains(k));
    }).toList();
  }

  /// Infers expenses based on context (e.g., "Hospital travel")
  static List<ExpenseItem> inferContext(List<ExpenseItem> expenses, String context) {
  ...
  static List<ExpenseItem> inferComplexIntent(List<ExpenseItem> expenses, String intent) {
    if (intent == 'hospital_travel') {
      ...
      return expenses.where((e) {
        ...
        final hasTravel = travelKeys.any((k) => text.contains(k)) || e.category?.toLowerCase() == 'travel';
